...
uses
  ...
  blcksock, pcnNFe, ACBrNFeNotasFiscais, System.math, acbrDFEUtil, ACBrDFeSSL,
...
begin
  ...
  begin
    ...
    NotaF.NFe.infRespTec.CNPJ := TiraPontos(Dados.qryRespTecnicoCNPJ.Value);
    NotaF.NFe.infRespTec.xContato := Dados.qryRespTecnicoNOME.Value;
    NotaF.NFe.infRespTec.email := Dados.qryRespTecnicoEMAIL.Value;
    NotaF.NFe.infRespTec.fone := TiraPontos(Dados.qryRespTecnicoTELEFONE.Value);
    NotaF.NFe.infRespTec.hashCSRT := Dados.qryRespTecnicoCSRT.Value;
  end;
    ...
    begin
      while SimpleRoundTo(vTotProd, -10) <> SimpleRoundTo(vSubTotal, -10) do
        begin
          ...
          //Somar os totais dos produtos -WS
...
//      SimpleRoundTo(qryItemTOTAL.AsFloat * qryItemALIQ_IPI.AsFloat / 100, -2);
  ...
  if Dados.qryEmpresaCRT.Value = 1 then // empresa do simples nacional
  begin
    if qryVendaFINALIDADE.Value = '0' then
    begin // finalidade normal
      if qryItemCSOSN.Value <> '202' then //Pedido de seu José 28/09/2021
        begin
