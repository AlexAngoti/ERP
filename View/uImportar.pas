...
begin
  ...
  if not FrmPDV.qryItem.IsEmpty then
  begin
    ShowMessage('Atenção! Não é possivel importar PRÉ-VENDA.' +
      #13'+ Venda atual possui ITENS cadastrados!');
    exit;
  end;
  ...
  if dados.qryConsulta.IsEmpty then
  begin
    ShowMessage('Pré-Venda não encontrado!');
    exit;
  end;
  ...
  FrmPDV.qryVendaOBSERVACOES.Value := 'Prevenda: ' + dados.qryConsulta.FieldByName
    ('CODIGO').AsString;
  ...
  if dados.qryConsulta.FieldByName('DESCONTO').AsFloat > 0 then
    begin
      ...
      FrmPDV.qryVendaDESCONTO.AsFloat :=
        dados.qryConsulta.FieldByName('DESCONTO').AsFloat;
    end;

  FrmPDV.qryVenda.Post;
  if not (qryPreVendaM.State in [dsEdit, dsInsert]) then
    qryPreVendaM.Edit;
  qryPreVendaMSITUACAO.AsString :=  'I';
  qryPreVendaM.Post;
  dados.Conexao.CommitRetaining;

  qryPreVendaD.Close;
  qryPreVendaD.Params[0].Value := dados.qryConsulta.FieldByName
    ('codigo').Value;
  qryPreVendaD.Open;

  {if FrmPDV.qryItem.IsEmpty then
    FrmPDV.AlimentaMemo(tpmIniciaVenda);}
...
procedure TfrmImportar.DBGrid2DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  {if qryPedidoPEDIDO_MOBILE.AsString = 'S' then // condição
