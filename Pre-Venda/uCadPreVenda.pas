...
type
  TFrmCadPreVenda = class(TForm)
    ...
    procedure qryPreMasterAfterOpen(DataSet: TDataSet);
    ...
    procedure qryPreDetalhePRECOValidate(Sender: TField);
    procedure qryPreDetalheQTDValidate(Sender: TField);
...
begin
  try
    ...
    qry.SQL.Clear;
    qry.SQL.Text  :=
      'UPDATE PRE_VENDA_DETALHE SET SITUACAO = ''C'' '+
  ...
  try
    ...
    if VDesconto > 0 then
      begin
        ...
        qryPreMasterTOTAL.AsFloat :=
          frmDescontoPreVenda.rResultadoDesconto;
        qryPreMaster.Post;
...
begin
  try
    ...
    qry.SQL.Clear;
    qry.SQL.Text  :=
      'UPDATE PRE_VENDA_DETALHE SET SITUACAO = ''F'' '+
      'WHERE FK_PREVENDA = ' + IntToStr(qryPreMasterCODIGO.AsInteger);
    qry.ExecSQL;
    qryPreDetalhe.Refresh;
  finally
  ...
  try
    ...
    on E: Exception do
      Application.MessageBox(PChar('Falha: '+sLineBreak+
        E.Message), 'Falha ao Inserir Item', 0+16);
  end;
end;
...
begin
  ...
  //Mudança para poder evitar erro ao inserir no vendas_master - WS
  for J := 0 to 3 do
    begin
      try
        qryPreMaster.Post;
        ...
        on E: Exception do
          begin
            if POS('violation of PRIMARY or UNIQUE KEY constraint', E.Message) > 0 then
              ...
              begin
                ShowMessage('Falha ao inserir pre-venda.'+sLineBreak+
...
begin
  try
    ...
    on E: Exception do
      begin
        Application.MessageBox(PChar('Falha: '+sLineBreak+
          E.Message), 'Localizar Produto', 0+16);
        qryPesqProd.Close;
      end;
  end;
end;
...
begin
  ...
  qryPreMasterTOTAL.AsFloat     := qrySomaSUBTOTAL.AsFloat - qrySomaDESCONTO.AsFloat;
  qryPreMaster.Post;
end;
...
begin
  qryPreDetalheTOTAL.AsFloat :=
    qryPreDetalheVALOR_ITEM.AsFloat - qryPreDetalheVDESCONTO.AsFloat;
end;

procedure TFrmCadPreVenda.qryPreDetalheNewRecord(DataSet: TDataSet);
begin
  qryPreDetalheCODIGO.Value :=
    dados.Numerador('PRE_VENDA_DETALHE', 'CODIGO', 'N', '', '');
...
procedure TFrmCadPreVenda.qryPreDetalhePRECOValidate(Sender: TField);
begin
  if not (qryPreDetalhe.State in dsEditModes) then
    qryPreDetalhe.Edit;
...
procedure TFrmCadPreVenda.qryPreDetalheQTDValidate(Sender: TField);
begin
  if not (qryPreDetalhe.State in dsEditModes) then
    qryPreDetalhe.Edit;
  qryPreDetalheVALOR_ITEM.AsFloat :=
    (RoundTo(qryPreDetalheQTD.AsFloat, -3) * RoundTo(qryPreDetalhePRECO.AsFloat, -3));
  qryPreDetalheTOTAL.AsFloat  :=  qryPreDetalheVALOR_ITEM.AsFloat -
...
procedure TFrmCadPreVenda.qryPreMasterAfterOpen(DataSet: TDataSet);
begin
  qryPreDetalhe.Close;
  qryPreDetalhe.Open;
end;
...
begin
  try
    PDesconto :=
      SimpleRoundTo(100 - ((qryPreMasterTOTAL.AsFloat * 100) /
    ...
    while not qry.Eof do
      begin
        VDescontoItem :=
          SimpleRoundTo((qry.FieldByName('VALOR_ITEM').AsFloat *
          PDesconto / 100), -2);
        ...
        qry.FieldByName('TOTAL').AsFloat :=
          qry.FieldByName('VALOR_ITEM').AsFloat - VDescontoItem;
        qry.Post;
        qry.Next;
      end;
      ...
      begin
        ...
        if VDesconto > VDescontoTot then
          begin
            ...
            VDescontoItem :=
              qry.FieldByName('VDESCONTO').AsFloat + VTemp;
            qry.FieldByName('TOTAL').AsFloat  :=
              qry.FieldByName('VALOR_ITEM').AsFloat - VDescontoItem;
          end
...
procedure TFrmCadPreVenda.SpeedButton1Click(Sender: TObject);
begin
  if qryPreDetalhe.State in dsEditModes then
    begin
      qryPreDetalhe.Post;
      FrmPrincipalPreVenda.MessageDlg('Produto Salvo!', 'Salvo!', tmConfirmado);
    end;
  edtBuscaProd.SetFocus;
end;
...
begin
  ...
  DBGrid1.Columns[2].Width := round(PageControl2.Width * 0.37); //VIRTUAL_PRODUTO
