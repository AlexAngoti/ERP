...
type
  ...
  TDados = class(TDataModule)
    ...
    procedure serial;
    procedure ConsultaCliente(var Codigo: Integer; var Razao: string; var Fantasia: string);
  ...
  public
    ...
    procedure AjustaPreco(produto, compra: Integer; preco, venda: Extended; bAjusteCompra: Boolean = False);
    ...
    procedure AjustaCusto(produto: Integer; compra, custo: Extended); // update 43
    ...
    procedure GerarBoleto(dm: TdmForte; var sPath: string;
      var sErro: string; bImprimir: Boolean = False);
  ...
  try
    ...
    then
      begin
        ...
        dados.qryParametroDATABASE_LI.AsString:=  dados.crypt('C', ''); //Nome do banco de dados
        dados.qryParametroUSUARIO_LI.AsString :=  dados.crypt('C', '');//Usuario do banco de dados
        dados.qryParametroSENHA_LI.AsString   :=  dados.crypt('C', ''); // Senha do banco de dados
        dados.qryParametroDATABASE_APP.AsString :=  dados.crypt('C', ''); //Nome do banco força de vendas
        dados.qryParametroUSUARIO_APP.AsString  :=  dados.crypt('C', ''); //Usuario do banco força de vendas
        dados.qryParametroSENHA_APP.AsString    :=  dados.crypt('C', ''); //Senha do banco força de vendas
...
procedure TDados.AjustaPreco(produto, compra: Integer; preco, venda: Extended; bAjusteCompra: Boolean);
var
  vMargem: Extended;
begin
  try
      ...
      begin
        ...
        if qryAjustaPrecoMARGEM.AsFloat > 0 then
          begin
            ...
            vMargem :=  vMargem *
              qryAjustaPrecoPR_CUSTO2.AsFloat;
            qryAjustaPrecoPR_VENDA.AsFloat :=
              qryAjustaPrecoPR_CUSTO2.AsFloat + vMargem
          end
        else
          begin
            qryAjustaPrecoPR_VENDA.AsFloat := SimpleRoundTo(venda, -2);
...
begin
  try
    ...
    qry.SQL.Clear;
    qry.SQL.Text  :=
      'select codigo from forma_pagamento fp '+
...
procedure TDados.ConsultaCliente(var Codigo: Integer; var Razao,
  Fantasia: string);
begin
  try
    try
...
function TDados.GravaUltimoPreco(IDProduto: Integer;
  UltimoPreco: Real): Boolean;
var
  qry: TFDQuery;
begin
  Result  := True;
  try
    ...
    try
      qry.Close;
      qry.SQL.Clear;
      qry.SQL.Text  :=  'insert into PROD_ULTIMOS_PRECOS (CODIGO, FKPRODUTO, ULTIMO_PRECO, DT_ULTIMO_PRECO, USUARIO)  ' +
                        'values (:CODIGO, :FKPRODUTO, :ULTIMO_PRECO, :DT_ULTIMO_PRECO, :USUARIO)  ';
      qry.ParamByName('CODIGO').Value           :=  Numerador('PROD_ULTIMOS_PRECOS', 'CODIGO', 'N', '', '');
      ...
      try
        ...
        qry.SQL.Clear;
        qry.SQL.Text  :=
          'drop table PROD_ULTIMOS_PRECOS;';
        qry.ExecSQL;
        qry.Close;
        qry.SQL.Clear;
        qry.SQL.Text  :=
          'CREATE TABLE PROD_ULTIMOS_PRECOS ( '+
          '    CODIGO SMALLINT, '+
          '    FKPRODUTO INTEGER, '+
          '    ULTIMO_PRECO DECIMAL(15,2), '+
          '    DT_ULTIMO_PRECO date, '+
          '    USUARIO VARCHAR(10)); ';
        qry.ExecSQL;
        qry.Close;
        qry.SQL.Clear;
...
begin
  try
    ...
    if not DirectoryExists(vPath) then
      ForceDirectories(vPath);
    vNamePDF  :=  'Boleto_'+Boleto.ListadeBoletos[0].NumeroDocumento+
      '_'+FormatDateTime('ddmmyyyy', Boleto.ListadeBoletos[0].Vencimento)+
      '_'+Boleto.ListadeBoletos[0].Sacado.CNPJCPF+'.pdf';
    ...
    on E: Exception do
      begin
        sErro :=  'Falha ao Gerar Boleto.'+sLineBreak+
          'Falha: '+sLineBreak+E.Message;
      end;
  end;
end;
...
begin
  try
    ...
    Boleto.ACBrBoletoFC.SoftwareHouse :=  'SystemDigitalERP';
...
procedure TDados.GravarNoGerenciadorBoleto(dm: TdmForte; sOrigem,
  sPath: string; iCodCReceber: Integer; iCodNFeFatura: Integer;
  CodigoBarras: string);
var
  Boleto: TACBrBoleto;
begin
  try
    try
      ...
      qryBoletoGerenciadorNOSSO_NUMERO.AsString         := Boleto.ListadeBoletos[0].NossoNumero;
      qryBoletoGerenciadorLINHA_DIGITAVEL.AsString      := CodigoBarras;
      qryBoletoGerenciadorvencimento.AsDateTime         := Boleto.ListadeBoletos[0].Vencimento;
      qryBoletoGerenciadordata_documento.AsDateTime     := Boleto.ListadeBoletos[0].DataDocumento;
      qryBoletoGerenciadornumero_documento.AsString     := Boleto.ListadeBoletos[0].NumeroDocumento;
      qryBoletoGerenciadordata_processamento.AsDateTime := Boleto.ListadeBoletos[0].DataProcessamento;
      qryBoletoGerenciadorvalor_documento.AsFloat       := Boleto.ListadeBoletos[0].ValorDocumento;
      qryBoletoGerenciadorsacado_nome_sacado.AsString   := Boleto.ListadeBoletos[0].Sacado.NomeSacado;
      qryBoletoGerenciadorsacado_cnpjcpf.AsString       := Boleto.ListadeBoletos[0].Sacado.CNPJCPF;
      qryBoletoGerenciadorsacado_logradouro.AsString    := Boleto.ListadeBoletos[0].Sacado.Logradouro;
      qryBoletoGerenciadorsacado_numero.AsString        := Boleto.ListadeBoletos[0].Sacado.Numero;
      qryBoletoGerenciadorsacado_bairro.AsString        := Boleto.ListadeBoletos[0].Sacado.Bairro;
      qryBoletoGerenciadorsacado_cidade.AsString        := Boleto.ListadeBoletos[0].Sacado.Cidade;
      qryBoletoGerenciadorsacado_uf.AsString            := Boleto.ListadeBoletos[0].Sacado.UF;
      qryBoletoGerenciadorsacado_cep.AsString           := Boleto.ListadeBoletos[0].Sacado.CEP;
      qryBoletoGerenciadorvalor_abatimento.AsFloat      := Boleto.ListadeBoletos[0].ValorAbatimento;
      qryBoletoGerenciadorvalor_mora_juros.AsFloat      := Boleto.ListadeBoletos[0].ValorMoraJuros;
      qryBoletoGerenciadorvalor_desconto.AsFloat        := Boleto.ListadeBoletos[0].ValorDesconto;
      qryBoletoGerenciadordata_mora_juros.AsDateTime    := Boleto.ListadeBoletos[0].DataMoraJuros;
      qryBoletoGerenciadordata_desconto.AsDateTime      := Boleto.ListadeBoletos[0].DataDesconto;
      qryBoletoGerenciadordata_abatimento.AsDateTime    := Boleto.ListadeBoletos[0].DataAbatimento;
      qryBoletoGerenciadordata_protesto.AsDateTime      := Boleto.ListadeBoletos[0].DataProtesto;
      qryBoletoGerenciadorpercentual_multa.AsFloat      := Boleto.ListadeBoletos[0].PercentualMulta;
      qryBoletoGerenciadorinstrucao1.AsString           := Boleto.ListadeBoletos[0].Instrucao1;
      qryBoletoGerenciadorinstrucao2.AsString           := Boleto.ListadeBoletos[0].Instrucao2;
  ...
  try
    ...
    try
      ...
      qry.SQL.Text  :=
        ...
        'where (p.codigo='+IntToStr(iCodCliente)+')';
      qry.Open;
      if not qry.IsEmpty then
        begin
          ...
          Titulo.Sacado.CNPJCPF    := OnlyNumber(SoNumero(qry.FieldByName('CNPJ').AsString));
          ...
          Titulo.LocalPagamento    := dados.qryBoletoParamMS_LOCAL_PAGAMENTO.AsString;
          ...
          Titulo.NossoNumero       := dm.ACBrBoleto.Banco.MontarCampoNossoNumero(Titulo);
         ...
         // dm.ACBrBoleto.AdicionarMensagensPadroes(Titulo,Mensagem);
          ...
          begin
            ...
            begin
              ...
              Titulo.Detalhamento.Add('Descrição Produto/Serviço ' +
                IntToStr(I) + ' '+ VLinha + ' Valor:   '+
                PadRight(FormatCurr('R$ ###,##0.00', StrToCurr(FloatToStr(rValor)) * 0.25),
                18,' ') );
            end;
    ...
    on E: Exception do
      begin
        Application.MessageBox(PChar('Falha ao incluir Boleto.'+
        sLineBreak+'Falha: '+sLineBreak+E.Message), 'Boleto',
        MB_OK+MB_ICONERROR);
      end;
  end;
...
begin
  try
    ...
    on E: Exception do
      begin
        Application.MessageBox(PChar('Falha ao registrar Boleto.'+
        sLineBreak+'Falha: '+sLineBreak+E.Message), 'Boleto',
        MB_OK+MB_ICONERROR);
      end;
  end;
